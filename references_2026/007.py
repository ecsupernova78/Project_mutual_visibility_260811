...
time_before_hours = 36.0
time_after_hours = 36.0
time_step_hours = 1
...
# %%
def generate_ra_dec_grid(
    ra_start_deg: float, ra_end_deg: float, dec_start_deg: float, dec_end_deg: float, ra_step_deg: float, dec_step_deg: float
...
def generate_interval_rect_plots(
    ...
    sirius_ra_deg = sirius.ra.deg
    sirius_dec_deg = sirius.dec.deg
    ...
    for idx, summary in enumerate(interval_summaries):
        ...
        # interval 대표 시각: midpoint
        mid_time = summary["start_time"] + 0.5 * (summary["end_time"] - summary["start_time"])
        sun = get_sun(mid_time)
        sun_ra_deg = sun.ra.deg
        sun_dec_deg = sun.dec.deg
        ...
        ax.scatter(sirius_ra_deg, sirius_dec_deg, marker=",", s=50, c="red",
            zorder=10, label="Sirius",
        )
        ax.text(
            sirius_ra_deg + 3,
            sirius_dec_deg + 3,
